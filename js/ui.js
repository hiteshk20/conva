...
export class UI {
    ...
    setupAssetLibrary() {
        ...
        shapes.forEach(s => {
            ...
            btn.onclick = () => {
                this.engine.addObject({ type: s.id, text: s.id === 'text' ? 'Double click to edit' : '' });
            };
            shapeContainer.appendChild(btn);
        });

        // Image upload
        const imgBtn = document.getElementById('upload-image');
        imgBtn.onclick = () => document.getElementById('file-input').click();
        ...
        fileInput.onchange = (e) => {
            ...
            if (file) {
                ...
                reader.onload = (event) => {
                    ...
                    // Load the actual image element for the engine to use
                    const img = new Image();
        ...
        const templates = ['socialPost', 'businessCard', 'modernSlide'];
        templates.forEach(t => {
            const btn = document.createElement('button');
            btn.className = 'p-2 text-xs bg-gray-200 hover:bg-gray-300 rounded mb-2 w-full transition-all text-gray-700 font-medium';
            btn.innerText = t.replace(/([A-Z])/g, ' $1').trim();
            btn.onclick = () => {
                // Import from Templates object (we'll handle the import in main.js or via a global)
    ...
    setupPropertyBar() {
        this.propBar = document.getElementById('property-bar');
        
        // Listen for selection changes by checking every frame or using a proxy.
        // For simplicity, we'll update the property bar in the main loop or via an event.
        this.updatePropertyBar();
    }

    updatePropertyBar() {
        const obj = this.engine.sceneGraph.find(o => o.id === this.engine.selectedId);
    ...
    updateSelected(prop, value) {
        if (this.engine.selectedId) {
            this.engine.updateObject(this.engine.selectedId, { [prop]: value });
        }
    }
    ...
    updateLayersPanel() {
        ...
        sorted.forEach(obj => {
            ...
            item.className = `flex items-center justify-between p-2 mb-1 rounded cursor-pointer transition-all ${this.engine.selectedId === obj.id ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50 text-gray-600'}`;
            item.innerHTML = `
    ...
    setupCanvasEvents() {
        ...
        canvas.onmousedown = (e) => {
            ...
            this.engine.handleMouseDown(e.clientX - rect.left, e.clientY - rect.top);
        };
